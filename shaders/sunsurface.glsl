// Based on https://www.shadertoy.com/view/XlSSzK#
...
float map(vec3 p) {
   ...
   R(p.xz, iMouse.x*0.008*pi+iGlobalTime*0.03);
...
vec3 firePalette(float i){
    ...
    return 1.0-exp(-3e8/L); // Exposure level. Set to "50." For "70," change the "5" to a "7," etc.
}
...
void main( void ){
  ...
  float r = (color.r * iOpacity);
	float g = (color.g * iOpacity);
	float b = (color.b * iOpacity);
  vec4 newColor = vec4(r, g, b, 1.0);
